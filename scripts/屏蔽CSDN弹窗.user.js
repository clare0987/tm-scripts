// ==UserScript==
// @name         屏蔽CSDN登录弹窗、登录后复制
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  屏蔽CSDN弹窗、登录后复制
// @author       宇宙蜗牛
// @match        *://*.csdn.net/*
...
(function() {
      ...
      if(mutations.type === 'childList' && mutations.addedNodes.length > 0) {
        mutations.addedNodes.forEach(node => {
          if(node.localName !== 'script') {
            const $closeBtn = $(node).find('.passport-login-box > span:contains("×")');
  ...
  // 去除不可复制样式
  $('pre').css('user-select', 'unset');
  $('code').css('user-select', 'unset');

  //移除“登陆后复制”按钮
  $('.hljs-button.signin').remove();
